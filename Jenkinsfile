pipeline{
    ...
    stages {
        ...
        stage('Deploy'){
            steps {  withCredentials([string(credentialsId: 'candySecretKey', variable: 'MY_SECRET_PASSWORD')]) {
                    sh "docker run -d --name=${BACK_SPRING_CONTAINER_NAME} -p 8080:8080 -e $JAVA_OPTS -jar myapp.jar --jasypt.encryptor.password=$MY_SECRET_PASSWORD ${BACK_SPRING_NAME}"}
